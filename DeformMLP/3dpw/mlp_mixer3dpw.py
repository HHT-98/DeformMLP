...
class MorphFC_ST(nn.Module):
    def __init__(self, dim, segment_dim_j=8, segment_dim_t=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.mlp_c = nn.Linear(dim, dim, bias=qkv_bias)
        #self.mix_h = DynaMixerOpx(dim, segment_dim_t, num_head=22, reduced_dim=2)
...
class PermutatorBlock(nn.Module):
    def __init__(self, dim, segment_dim_j, segment_dim_t, num_head_j, num_head_t, reduced_dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=Aff, skip_lam=1.0, mlp_fn=MorphFC_ST):
        super().__init__()
    ...
    def forward(self, x):  # B,T,J,C
        ...
        x_t = x + self.drop_path(self.t_fc(self.norm1(x))) /self.skip_lam
        x_s = x_t + self.drop_path(self.s_fc(self.norm2(x_t))) /self.skip_lam
        x_st = x + self.drop_path(self.fc(self.norm3(x_s))) / self.skip_lam
        x_c = x_st + self.drop_path(self.mlp(self.norm4(x_st))) / self.skip_lam
        return x_c
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=1, padding=1,stride=1)
...
class MorphMLP(nn.Module):
    ...
    def __init__(self):
        ...
        segment_dim_j = [25, 25, 25, 25]
        segment_dim_t = [15, 15, 15, 15]
        mlp_ratios = [3, 3, 3, 3]
        mlp_dim_t = [11, 11, 11, 11]
        mlp_dim_j = [5, 5, 5, 5]
        reduced_dim = [2, 2, 2, 2]
        embed_dims = [75, 75, 75, 75]
        patch_size = 7
        qkv_bias = False
        qk_scale = None
        attn_drop_rate = 0.1
        ...
        self.fc_out1 = nn.Linear(75, 69)
        self.conv_out = nn.Conv1d(15, 25, 1, stride=1)
    ...
    def forward(self, x):  # 224,224,T,3
        x = x.cpu().numpy()
        x_copy = np.repeat(x[:,-1:,:], 3, axis=1)
        x =np.concatenate([x, x_copy], axis=1)
        x = torch.from_numpy(x).cuda()
        x = x.reshape(-1, 13, 23, 3)#B,T,J,C
        x = x.permute(0, 3, 1, 2)
        x = self.forward_features(x)
        x = self.norm(x)#B,10,22,110
        x = self.fc_out(x)
        x = self.conv_out(x.reshape(-1, 15, 75))
        x = self.fc_out1(x)
